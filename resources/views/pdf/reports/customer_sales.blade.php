...
<body>
    ...
    <div class="invoice">
        <h2 class="heading text-center">{{ __('messages.customer_sales_report') }}</h2>
                ...
                <table width="100%">
                    <tbody>
                        @foreach ($customer->invoices as $invoice)
                            <tr>
                                <td>{{ $invoice->formatted_invoice_date }} ({{ $invoice->invoice_number }})</td>
                                <td colspan="2"></td>
                                <td align="right">{!! money($invoice->total, $invoice->currency_code) !!}</td>
                            </tr>
                        @endforeach
                    </tbody>
                    <tfoot class="border-top">
                        <tr>
                            <td colspan="3"></td>
                            <td align="right">
                                {{ __('messages.total') }}: {!! money($customer->totalAmount, $customer->currency_code) !!}
                            </td>
                        </tr>
                    </tfoot>
                </table>
