...
<body>
    ...
    <div class="invoice">
        <h2 class="heading text-center">{{ __('messages.profit_loss_report') }}</h2>

        <h3>{{ __('messages.income') }}</h3>
        <table width="100%" class="border-top">
            <tbody>
                <tr>
                    <td>{{ __('messages.total') }}</td>
                    <td colspan="2"></td>
                    <td align="right">{!! money($invoices_total, $company->currency->short_code) !!}</td>
                </tr>
            </tbody>
        </table>  
        ...
        <table width="100%">
            <tbody>
                @foreach ($expense_categories as $expense_category)
                    <tr>
                        ...
                        <td colspan="2"></td>
                        <td align="right">{!! money($expense_category->total_expense, $company->currency->short_code) !!}</td>
                    </tr>
                @endforeach
            </tbody>
            <tfoot class="border-top">
                <tr>
                    <td colspan="3"></td>
                    <td align="right">
                        {{ __('messages.total') }}: {!! money($total_loss, $company->currency->short_code)->format() !!}
                    </td>
                </tr>
            </tfoot>
        </table>
        ...
        <h3>{{ __('messages.net_profit') }}</h3>
        <table width="100%" class="border-top">
            <tbody>
                <tr>
                    <td>{{ __('messages.total') }}</td>
                    <td colspan="2"></td>
                    <td align="right">{!! money($invoices_total - $total_loss, $company->currency->short_code)->format() !!}</td>
                </tr>
            </tbody>
        </table> 
    </div>
