    ...
    <div class="row no-gutters card-form__body card-body bg-white">
            ...
            <div class="form-group required select-container">
                ...
                <select id="customer" name="customer_id" data-toggle="select" class="form-control select2-hidden-accessible select-with-footer" data-select2-id="customer">
                    ...
                    @if($credit_note->customer_id)
                        <option value="{{ $credit_note->customer_id }}" 
                            selected=""
                            data-currency="{{ $credit_note->customer->currency }}" 
                            data-billing_address="{{$credit_note->customer->displayLongAddress('billing')}}"
                            data-shipping_address="{{$credit_note->customer->displayLongAddress('shipping')}}"
                            >
        ...
        <div class="col-md-4 pr-4 pl-4">
            <div class="form-group required">
                <label for="credit_note_date">{{ __('messages.credit_note_date') }}</label>
                <input name="credit_note_date" type="text" class="form-control input" data-toggle="flatpickr" data-flatpickr-default-date="{{ $credit_note->credit_note_date ?? now() }}" readonly="readonly" required>
            </div>
            <div class="form-group required"> 
                <label for="credit_note_number">{{ __('messages.credit_note_number') }}</label>
                <div class="input-group input-group-merge">
                    <input name="credit_note_prefix" type="hidden" value="{{ $credit_note->credit_note_prefix }}">
                    <input name="credit_note_number" type="text" maxlength="6" class="form-control form-control-prepended" value="{{ $credit_note->credit_note_num }}" autocomplete="off" required>
        ...
        <div class="col-md-4 pl-4">
            <div class="form-group">
                ...
                <div class="input-group input-group-merge">
                    <input name="reference_number" type="text" maxlength="6" class="form-control form-control-prepended" value="{{ $credit_note->reference_number }}" autocomplete="off">
                    <div class="input-group-prepend">
                        <div class="input-group-text">
                            #
                        </div>
        ...
        <div class="col-md-5 mt-5 pr-4">
            <div class="form-group">
                ...
                <textarea name="notes" class="form-control" rows="4">{{ $credit_note->notes }}</textarea>
            </div>

            <div class="form-group">
                <label for="private_notes">{{ __('messages.private_notes') }}</label>
                <textarea name="private_notes" class="form-control" rows="4">{{ $credit_note->private_notes }}</textarea>
            </div>

            <div class="form-group">
                <label for="template_id">{{ __('messages.credit_note_template') }}</label>
                <select id="template_id" name="template_id" data-toggle="select" class="form-control" data-select2-id="template_id">
                    @foreach(\App\Models\CreditNoteTemplate::all() as $template)
                        <option value="{{ $template->id }}" {{ $credit_note->template_view == $template->view ? 'selected' : '' }}>{{ $template->name }}</option>
                    @endforeach
                </select> 
            </div>
        </div>
            ...
            <div class="card card-body shadow-none border">
                ...
                <div class="d-flex align-items-center mb-3">
                    ...
                    <div class="ml-auto h6 mb-0">
                        <input id="sub_total" name="sub_total" type="text" class="price_input price-text w-100 fs-inherit" value="{{ $credit_note->sub_total ?? 0 }}" readonly>
                    </div>
                </div>
                        ...
                        <div class="col-12 h6 mb-0">
                            <div class="form-group select-container">
                                <select id="total_taxes" name="total_taxes[]" data-toggle="select" multiple class="form-control priceListener select-with-footer" data-select2-id="total_taxes">
                                    @foreach(get_tax_types_select2_array($currentCompany->id) as $option)
                                        <option value="{{ $option['id'] }}" data-percent="{{ $option['percent'] }}" {{ $credit_note->hasTax($option['id']) ? 'selected=""' : '' }}>{{ $option['text'] }}</option>
                                    @endforeach
                                </select> 
                                <div class="d-none select-footer">
                                    <a href="{{ route('settings.tax_types.create', ['company_uid' => $currentCompany->uid]) }}" target="_blank" class="font-weight-300">+ {{ __('messages.add_new_tax') }}</a>
                ...
                @if($discount_per_item == false)
                    <div class="row mt-2 mb-1">
                        ...
                        <div class="col-12 h6 mb-0">
                            <div class="form-group">
                                <div class="input-group input-group-merge">
                                    <input id="total_discount" name="total_discount" type="number" class="form-control form-control-prepended priceListener" value="{{ $credit_note->discount_val ?? 0 }}">
                                    <div class="input-group-prepend">
                                        <div class="input-group-text">
                                            %
                                        </div>
                ...
                <div class="d-flex align-items-center mb-3">
                    ...
                    <div class="ml-auto h5 mb-0">
                        <input id="grand_total" name="grand_total" type="text" class="price_input price-text w-100 fs-inherit" value="{{ $credit_note->total ?? 0 }}" readonly>
                    </div>
                </div>
            </div>
        </div>

        @if($credit_note->getCustomFields()->count() > 0)
            <div class="col-12">
                @foreach ($credit_note->getCustomFields() as $custom_field)
                    @include('layouts._custom_field', ['model' => $credit_note, 'custom_field' => $custom_field])
                @endforeach
            </div>
        @endif
 
        <div class="col-12 text-center float-right mt-3">
            <button type="button" class="btn btn-primary save_form_button">{{ __('messages.save_credit_note') }}</button>
        </div>
    </div>
</div>
