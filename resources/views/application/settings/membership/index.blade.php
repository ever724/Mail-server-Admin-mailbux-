        ...
        <div class="card card-form">
            <div class="row no-gutters">
                <div class="col-12 col-md-6 card-form__body card-body bg-white">
                    ...
                    <div class="font-weight-bold">{{ __('messages.plan') }}: <p class="font-weight-normal">{{ $subscription->plan->name }}</p>
                    </div>
                    <div class="font-weight-bold">{{ __('messages.status') }}: <p class="font-weight-normal">{!! $subscription->html_status !!}</p>
                    </div>
                    @if($subscription->onTrial())
                    <div class="font-weight-bold">{{ __('messages.trial_ends') }}: <p class="font-weight-normal">{{ $subscription->trial_ends_at->format($dateFormat) }}</p>
                    </div>
                    @else
                    <div class="font-weight-bold">{{ __('messages.expiry_date') }}: <p class="font-weight-normal">{{ $subscription->ends_at->format($dateFormat) }}</p>
                    </div>
                    @endif
                ...
                <div class="table-responsive">
                    <table class="table mb-0 thead-border-top-0 table-striped">
                        ...
                        <tbody class="list" id="orders">
                            ...
                            <tr>
                                <td>
                                    <a class="mb-0" target="_blank" href="{{ route('settings.membership.invoice', ['company_uid' => $currentCompany->uid, 'order_id' => $order->order_id]) }}">{{ $order->order_id }}</a>
                                </td>
                                <td>
                                    @if($order->plan)
                                ...
                                <td><i class="material-icons icon-16pt text-muted-light mr-1">today</i> {{ $order->created_at->format($dateFormat) }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
